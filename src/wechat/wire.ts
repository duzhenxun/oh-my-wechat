...
import { loadAccount, type OmwAccount } from "./login.ts";
import {
...
type HttpLogContext = {
  ...
  enabled?: boolean;
  shouldLog?: (payload: unknown) => boolean;
};
...
function writeHttpLog(context: HttpLogContext | undefined, title: string, payload: unknown): void {
...
export class WechatWire {
  ...
  private readonly httpLogEnabled: boolean;
  ...
  async poll(timeoutMs = 35_000, minCreatedAtMs?: number): Promise<{ messages: IncomingWechatText[]; ignored: number }> {
    ...
    }, timeoutMs, this.updatesHttpLogContext())) as Updates;
  ...
  private httpLogContext(): HttpLogContext {
    return {
      log: this.log,
  ...
  private updatesHttpLogContext(): HttpLogContext {
    return {
      ...this.httpLogContext(),
      shouldLog: (payload) => {
        ...
        try {
          const parsed = JSON.parse(body) as Updates;
  ...
  private logHttp(title: string, payload: unknown): void {
  ...
  private async sendItems(account: OmwAccount, to: string, contextToken: string, itemList: unknown[]): Promise<void> {
    const result = JSON.parse(await postJson(account, "ilink/bot/sendmessage", {
      ...
      base_info: { channel_version: WIRE_VERSION },
    }, 15_000, this.httpLogContext())) as WechatApiResult;
  ...
  private async upload(account: OmwAccount, to: string, filePath: string, label: "file" | "image" | "voice" | "video"): Promise<{ rawSize: number; encryptedSize: number; aesKey: Buffer; downloadParam: string }> {
    ...
    const uploadInfo = JSON.parse(await postJson(account, "ilink/bot/getuploadurl", {
      ...
      no_need_thumb: true,
      base_info: { channel_version: WIRE_VERSION },
    }, 15_000, this.httpLogContext())) as WechatApiResult & { upload_param?: string };
