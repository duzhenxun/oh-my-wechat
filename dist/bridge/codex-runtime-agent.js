...
function extractThreadEventId(value) {
    const record = asRecord(value);
    return firstString(record.threadId, record.thread_id, asRecord(record.thread).id, asRecord(record.thread).threadId, asRecord(record.thread).thread_id);
}
function extractTurnEventId(value) {
    const record = asRecord(value);
    return firstString(record.turnId, record.turn_id, asRecord(record.turn).id, asRecord(record.turn).turnId, asRecord(record.turn).turn_id);
}
...
function extractFinalText(item) {
...
function approvalPreview(params) {
    const record = asRecord(params);
...
function matchesPendingApproval(record, pending) {
    if (!pending) {
        return false;
    }
...
//# sourceMappingURL=codex-runtime-agent.js.map