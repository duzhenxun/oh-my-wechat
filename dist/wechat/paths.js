...
export const ILINK_BASE_URL = process.env.OH_MY_WECHAT_ILINK_BASE_URL?.trim() ||
...
export function writeCodexRuntimeEndpoint(endpoint) {
    ensureOmwHome();
...
export function readCodexRuntimeEndpoint(cwd) {
    return readJson(codexRuntimePath(cwd));
}
export function clearCodexRuntimeEndpoint(cwd) {
    fs.rmSync(codexRuntimePath(cwd), { force: true });
}
export function clearCodexRuntimeEndpointForPid(pid) {
    ...
    for (const name of fs.readdirSync(CODEX_RUNTIME_DIR)) {
        ...
        const endpoint = readJson(filePath);
        if (endpoint?.bridgePid === pid) {
            fs.rmSync(filePath, { force: true });
        }
    }
}
export function readActiveCodexRuntimeEndpoint(cwd) {
...
export function readBridgeLock() {
    return readJson(BRIDGE_LOCK_PATH);
}
...
export function killBridgeProcess(pid) {
    try {
...
export function listOtherBridgePids(currentPid) {
    try {
...
export async function killOtherBridges(currentPid, waitMs = 4_000) {
...
export async function acquireBridgeLock(lock, options) {
    ensureOmwHome();
    ...
    for (;;) {
        try {
            ...
            if (!replaceExisting) {
                throw new Error(`Another bridge is already running (pid ${existing.pid}, mode=${existing.mode}, cwd=${existing.cwd}). Stop it before starting a new one.`);
            }
            ...
            if (isProcessAlive(existing.pid)) {
                throw new Error(`Could not stop existing bridge (pid ${existing.pid}, mode=${existing.mode}, cwd=${existing.cwd}).`);
            }
            fs.rmSync(BRIDGE_LOCK_PATH, { force: true });
        }
    }
}
export function releaseBridgeLock(pid) {
    const existing = readBridgeLock();
